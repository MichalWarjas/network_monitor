...
def start_sniffing(interface=None):
    ...
    global captured_packets, stop_sniffing_event
    captured_packets = [] # Reset packets when starting
    ...
    try:
        ...
        # The 'stop_filter' uses the threading event to stop sniffing
        # The 'iface' argument specifies the network interface
        sniff(prn=packet_callback, store=0, stop_filter=lambda p: stop_sniffing_event.is_set(), iface=interface)
        print("Sniffing stopped.")
    except OSError as e:
        # Handle potential errors like permission denied or invalid interface
...
def stop_sniffing():
    ...
    if capture_thread and capture_thread.is_alive():
        ...
        capture_thread.join(timeout=2) # Wait for the thread to finish
...
# Get available network interfaces (requires psutil)
try:
    import psutil
...
while True:
    with placeholder.container():
        ...
        # Wait for the refresh interval before rerunning the loop
        time.sleep(refresh_interval)

        # Rerun the Streamlit script to refresh the data
        # Note: This relies on Streamlit's execution model.
        # For continuous updates without full reruns, more complex state management or callbacks might be needed.
        st.rerun() # Rerun the script to update the dashboard

# Ensure sniffing stops if the Streamlit app is closed or interrupted
# This might not always be reliable depending on how the script exits.
# A more robust solution might involve try/finally blocks around the main loop.
# stop_sniffing() # Attempt to stop sniffing on exit (may not execute reliably)
